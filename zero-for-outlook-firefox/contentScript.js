      ...
      browserApi.storage.sync.get(
        ...
        (items) => {
          ...
          if (items) {
            ...
            // Rebuild commands list with custom shortcuts
            if (typeof rebuildCommandList === "function") {
              rebuildCommandList();
            }
          } else {
            customShortcuts = [];
...
`;
  ...
  let aiEditingOverlay = null;

  function ensureAiEditingStyles() {
...
.oz-ai-editing-backdrop.oz-ai-editing-dark {
  background:
    radial-gradient(circle at top left, rgba(59, 130, 246, 0.25), transparent 55%),
    radial-gradient(circle at bottom right, rgba(129, 140, 248, 0.3), transparent 55%),
    rgba(15, 23, 42, 0.32);
...
.oz-ai-editing-backdrop.oz-ai-editing-light {
  background:
    radial-gradient(circle at top left, rgba(59, 130, 246, 0.18), transparent 55%),
    radial-gradient(circle at bottom right, rgba(129, 140, 248, 0.2), transparent 55%),
    rgba(15, 23, 42, 0.12);
...
.oz-ai-editing-modal.oz-ai-editing-dark {
  background-color: #020617;
  box-shadow:
    0 18px 36px rgba(15, 23, 42, 0.9),
    0 0 0 1px rgba(148, 163, 184, 0.5);
...
.oz-ai-editing-modal.oz-ai-editing-light {
  background-color: #ffffff;
  box-shadow:
    0 10px 24px rgba(15, 23, 42, 0.16),
    0 0 0 1px rgba(148, 163, 184, 0.45);
...
`;
  ...
  function closeAiEditingOverlay() {
    if (aiEditingOverlay && aiEditingOverlay.parentNode) {
      aiEditingOverlay.parentNode.removeChild(aiEditingOverlay);
    }
    aiEditingOverlay = null;
  }

  function openAiEditingOverlay() {
    ensureAiEditingStyles();
    ...
    modal.innerHTML = `
      ...
      <div class="oz-ai-editing-message">
        Using AI to format your shortcut name...
      </div>
    `;
...
.oz-element-picker-instructions {
  position: fixed;
  top: 20px;
  left: 50%;
  transform: translateX(-50%);
...
`;
  ...
  function executeCustomShortcut(customShortcut) {
  ...
  function selectElement(elementToSave) {
    ...
    if (!aiTitleEditingEnabled) {
      // AI title editing is disabled, use fallback immediately
    ...
    closeElementPicker();
    openAiEditingOverlay();

    requestElementDescriptionFormat(elementInfo).then((result) => {
      // Close AI editing overlay before saving
      closeAiEditingOverlay();
  ...
  try {
    browserApi.storage.onChanged.addListener((changes, areaName) => {
      ...
      if (changes.customShortcuts && Array.isArray(changes.customShortcuts.newValue)) {
        ...
        rebuildCommandList();
        // Re-render command list if overlay is open
        if (commandOverlay && commandInput) {
          renderCommandList(commandInput.value || "");
        }
      }
